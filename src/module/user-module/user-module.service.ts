...
import {
	ApiResponse,
	File,
} from "@src/_types/api-response.type";
...
export class UserModuleService {
	async create(
		data: UserModuleEntity
	): Promise<ApiResponse> {
		const findUser = await userModuleModel
			.findOne({
				email: data.email,
			})
			.exec();
	...
	async sendRecoverPasswordLink(data: {
		email: string;
	}): Promise<ApiResponse> {
		const transporter =
		...
		await transporter.sendMail({
			...
			text:
				"Para recuperar sua senha, acesse o seguinte link: " +
				environment.frontEndUrl +
				`/auth/password-reset?token=${token}&email=${data.email}`,
		});
	...
	async updateMe(
		...
		if (imageFile) {
			...
			try {
				await new S3Provider()
					.getS3()
					.send(command);
				imageUrl = `${environment.publicImageDevUrl}/${environment.bucketName}/${command.input.Key}`;
