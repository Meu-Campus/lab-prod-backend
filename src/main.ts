  ...
  private buildRoutes(router: Router): Router {
    controllers.forEach(controller => {
      ...
      routeConfigs.forEach((routeConfig: RouteConfig) => {
        ...
        switch (method) {
          case "get": router.get(path, ...jobs); break;
          case "post": router.post(path, ...jobs); break;
          case "put": router.put(path, ...jobs); break;
          case "delete": router.delete(path, ...jobs); break;
        }
      });
    });
    return router;
